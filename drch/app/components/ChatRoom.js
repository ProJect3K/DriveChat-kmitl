...
const ChatRoom = ({
  ...
  // Extract room display name from room ID
  ...
  return (
    <div className="relative min-h-screen bg-gray-100 p-4">
      ...
      {/* Main Chat Area */}
      <div className="bg-white rounded-lg shadow-lg p-4 min-h-[500px] relative">
        {/* Chat Header */}
        <div className="border-b pb-2 mb-4">
          <h2 className="text-xl font-bold">Room: {getRoomDisplayName()}</h2>
          <div className="text-sm text-gray-600">
            user seat : {activeUsers.length}/{roomCapacity}
          </div>
        </div>

        {/* Messages Area */}
        <div className="space-y-4 h-[400px] overflow-y-auto mb-4 px-4">
          {messages.map((msg, index) => {
            ...
            // Skip rendering the ROOM_CHANGE system messages
